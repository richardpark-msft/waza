...
func runCommandE(cmd *cobra.Command, args []string) error {
	...
	// if the user hasn't explicitly chosen skill paths, load the skills
	// from their .waza.yaml

	// TODO: okay, wait, how does it know the path to start discovering from?
	if cfg.Paths.Skills != "" {
		slog.Info("Workspace has a skills folder", "folder", cfg.Paths.Skills)
		discoveredSkills, err := discovery.Discover(cfg.Paths.Skills)
		...
		slog.Info("Workspace skills added", "skills", skillFolders)
	}
